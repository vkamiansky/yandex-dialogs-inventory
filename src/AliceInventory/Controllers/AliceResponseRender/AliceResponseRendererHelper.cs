...
namespace AliceInventory.Controllers.AliceResponseRender
{
    public static class AliceResponseRendererHelper
    {
        ...
        private static readonly ResponseTemplate RequestMailTemplate = new ResponseTemplate
        {
            ...
            {
                new TextAndSpeechTemplate(
                    "К сожалению, я не знаю, какая у Вас почта, но если вы скажете, я запомню и никому не скажу. Какая у вас почта?")
            },
            Buttons = MainButtons
        };
        ...
        private static readonly ResponseTemplate MailAddedTemplate = new ResponseTemplate
        {
            TextAndSpeechTemplates = new[]
            {
                new TextAndSpeechTemplate("Я добавила почту \"{0}\". Отправить на неё отчет?",
                    "Я добавила почту. Отправить на неё отчет?"),
                new TextAndSpeechTemplate("Я сохранила почту \"{0}\". Отправить на неё отчет?",
                    "Я сохранила почту. Отправить на неё отчет?")
            },
            Buttons = MainButtons
        };
        ...
        private static Response CreateResponse(ResponseArgs args, CultureInfo cultureInfo,
            Func<int, int> variantSelector)
        {
